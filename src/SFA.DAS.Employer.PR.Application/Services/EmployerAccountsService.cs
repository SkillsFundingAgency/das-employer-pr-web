...
public class EmployerAccountsService : IEmployerAccountsService
{
    ...
    private static EmployerUserAccounts Transform(GetEmployerUserAccountsResponse response) =>
        new(response.IsSuspended, response.FirstName, response.LastName, response.EmployerUserId, response.UserAccounts.Select(u => new EmployerIdentifier(u.EncodedAccountId, u.DasAccountName, u.Role!)).ToList());
}
