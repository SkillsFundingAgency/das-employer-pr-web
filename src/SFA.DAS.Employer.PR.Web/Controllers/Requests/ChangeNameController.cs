...
public class ChangeNameController(IOuterApiClient _outerApiClient, ISessionService _sessionService, IValidator<EmployerAccountNamesSubmitModel> _validator) : Controller
{
    ...
    public async Task<IActionResult> Index(Guid requestId, CancellationToken cancellationToken)
    {
        ...
        EmployerAccountNamesModel vm = GetChangeNameViewModel(permissionRequest.EmployerContactFirstName!, permissionRequest.EmployerContactLastName!, backLink!);
    ...
    public IActionResult Index(Guid requestId, EmployerAccountNamesSubmitModel submitModel, CancellationToken cancellationToken)
    {
        ...
        if (!result.IsValid)
        {
            EmployerAccountNamesModel model = GetChangeNameViewModel(submitModel.EmployerContactFirstName!,
                submitModel.EmployerContactLastName!,
                Url.RouteUrl(RouteNames.CreateAccountCheckDetails, new { requestId })!);
            result.AddToModelState(ModelState);
            return View(RequestsChangeNameViewPath, model);
        }
    ...
    private static EmployerAccountNamesModel GetChangeNameViewModel(string firstName, string lastName, string backLink)
    {
        return new EmployerAccountNamesModel
        {
