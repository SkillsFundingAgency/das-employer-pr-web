...
public class YourTrainingProvidersControllerTests
{
    ...
    //MFCMFC temporarily commented out
    // [Test]
    ...
    [Test, MoqInlineAutoData]
    public void SessionModelSuccessfulAddition_AddsShowBannerAndProviderNameToViewModel(
        [Frozen] Mock<IOuterApiClient> outerApiMock,
        [Frozen] Mock<ISessionService> sessionServiceMock,
        string employerAccountId,
        string providerName,
    ...
    )
    {
        ...
        var viewResult = result.Result.As<ViewResult>();
        var viewModel = viewResult.Model as YourTrainingProvidersViewModel;
        viewModel!.ShowPermissionsJustAddedBanner.Should().BeTrue();
        viewModel.ProviderWithChangedPermissions.Should().Be(providerName);
    }
